...
**CHECKPOINT 2.5 - LOOK AT THE PIXELS (mandatory; added after the M9.2 black-frame
postmortem).** Files existing is not the same as the scene rendering. Verify content
before spending another GPU-minute (install if needed:
`python3 -m pip install --user pillow numpy`):

```bash
python3 -c "import glob; from PIL import Image; import numpy as np; a = np.asarray(Image.open(sorted(glob.glob('kit/out/r1_ref/rgb_*.png'))[0])); print('mean pixel value:', round(float(a.mean()), 2))"
```

Mean > 10: scene is lit and visible - proceed to the matrix. Mean < 2: BLACK FRAMES -
STOP; diagnose lighting/camera before any further spend. Also scp one frame home and
open it with human eyes - 30 seconds that would have saved the first session.

Note (day 2): scripts are now natively standalone (bootstrap + completion footer
generated by the adapter) - the live patch step from session 1 is no longer needed;
the container invocation uses `--entrypoint /isaac-sim/python.sh` and image tag 4.5.0.
